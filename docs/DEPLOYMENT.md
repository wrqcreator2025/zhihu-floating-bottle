...
验证步骤：知乎登录后检查「✓ 知乎已登录」；打开空瓶填写问题，点击「发出瓶子」，服务端确认提交后应显示「瓶子已发出，审核后开始寻找」；也可选择「先让 AI 帮我整理（可选）」，确认建议后发送；进入瓶子柜刷新实际审核和寻找状态；在日记中保存本人经历并开放接收。需要另一个真实账号填写符合条件的经历，才能验证接收、回信和聊天。没有匹配人时不会生成 AI 回信。
...
`AI_PROTOCOL_ERROR` 表示模型结果无法按约定读取，适配器会自动重试一次；可按 API/worker 日志中的 `AI response rejected` 查看 `task` 和 `reason`。前端整理失败时保留原稿并提供直接发送入口；直接发送成功仅表示提交成功，后续审核、匹配仍需要有效模型配置及运行中的 worker。
