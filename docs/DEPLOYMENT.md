...
部署时 API 和 worker 都配置相同的 `ZHIHU_ACCESS_SECRET`，即可同时用于知乎搜索和知乎直答。未显式设置三项 `AI_*` 变量时，后端自动使用 `https://developer.zhihu.com/v1` 和 `zhida-fast-1p5`；若 Railway 中已有占位或旧 `AI_*` 值，请删除三项，确保默认配置生效。仅部署前端或 API 不会运行后台匹配及消息审核任务，必须同时启动 worker。

验证步骤：知乎登录后检查「✓ 知乎已登录」；打开空瓶填写问题，点击「发出瓶子」，服务端确认提交后应显示「瓶子已发出，正在寻找有相似经历的人」；也可选择「先让 AI 帮我整理（可选）」，确认建议后发送；进入瓶子柜刷新实际寻找状态；在日记中保存本人经历并开放接收。需要另一个真实账号填写符合条件的经历，才能验证接收、回信和聊天。没有匹配人时不会生成 AI 回信。
