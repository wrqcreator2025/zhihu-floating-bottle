...
抛瓶成功返回 `searching`；worker 不对漂流瓶内容进行审核，会先用 AI 根据原文、提示和可用活动画像识别主题，再用于推荐匹配。若后台 AI 识别或排序暂时不可用，会降级为更宽的候选投递，尽量避免把瓶子置为 `search_error`；不会产生伪造回信。旧版本的瓶子审核记录仅保留作历史记录，不再阻止新一轮匹配。
